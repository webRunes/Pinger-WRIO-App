...
function createImage(text, done) {
    var lineHeight = 14;
    var canvas = new _canvas2.default(500, 650);
    ...
    ctx.fillStyle = "#aaa";
    ctx.fillText('Posted via Titter — Advanced tweets, visit titter.wrioos.com', 10, ms_height);
    ...
    canvas.toBuffer(function (err, buf) {
        ...
        temp.open({
            ...
            _fs2.default.write(file.fd, buf, 0, buf.length, null, function (err, written, buffer) {
                if (err) {
                    console.log("Error closing file");
                    done(null);
                    return;
                }
                _fs2.default.close(file.fd, function () {
                    console.log("Image written to tmp file ", file.path);
                    done(file.path);
                });
            });
            _fs2.default.close(file.fd, function (err) {});
        });
    });
}
...
titterPicture.drawCommentP = (0, _es6Promisify2.default)(titterPicture.drawComment); // promisified version
